...
class model_append_dlg(QDialog):

    def __init__(self):
        QDialog.__init__(self)
        self.ui = uic.loadUi("setting/dialog_model.ui", self)
        ...
        self.model_append.clicked.connect(self.model_append_btn_click)

        self.normal_shutdown_token = False

        # date setting
        today = self.win_request_date.selectedDate()
        self.win_request_date.setSelectedDate(today.addDays(1))

        # if next day is sunday, then pass
        if today.dayOfWeek() == 7:
            self.win_request_date.setSelectedDate(today.addDays(2))
    ...
    def model_append_btn_click(self):
        try:
            model_list = []
            ...
            text = "Error raise in model window \n" + str(ex)
            self.textEdit.setText(text)


if __name__ == "__main__":
    app = QApplication(sys.argv)
    myWindow = model_append_dlg()
    myWindow.show()
    app.exec_()